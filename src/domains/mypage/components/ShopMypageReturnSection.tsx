...
import type { ShopMypageReturnHistoryPageResponse } from "@/domains/mypage/types";
import { formatShopMypageOrderCount } from "@/domains/mypage/utils/shopMypageOrder";
import ShopMypageReturnCardList from "./ShopMypageReturnCardList";
import styles from "./ShopMypageCancelSection.module.css";

type ShopMypageReturnRange = "1m" | "3m" | "6m" | "custom";

interface ShopMypageReturnSectionProps {
  returnPageData: ShopMypageReturnHistoryPageResponse;
  initialRange: ShopMypageReturnRange;
}

interface ShopMypageReturnPaginationState {
...
function resolvePaginationState(pageNo: number, totalPageCount: number): ShopMypageReturnPaginationState {
...
// 반품내역 페이지 링크를 생성합니다.
function buildMypageReturnHref(range: ShopMypageReturnRange, startDate: string, endDate: string, pageNo: number): string {
  const queryParams = new URLSearchParams();
  queryParams.set("range", range);
  queryParams.set("startDate", startDate);
  queryParams.set("endDate", endDate);
  queryParams.set("pageNo", String(Math.max(Math.floor(pageNo), 1)));
  return `/mypage/return?${queryParams.toString()}`;
}

// 마이페이지 반품내역 화면을 렌더링합니다.
export default function ShopMypageReturnSection({ returnPageData, initialRange }: ShopMypageReturnSectionProps) {
  const router = useRouter();
  const [selectedRange, setSelectedRange] = useState<ShopMypageReturnRange>(initialRange);
  const [startDateInputValue, setStartDateInputValue] = useState(returnPageData.startDate);
  const [endDateInputValue, setEndDateInputValue] = useState(returnPageData.endDate);

  // 현재 페이지네이션 상태를 계산합니다.
  const paginationState = resolvePaginationState(returnPageData.pageNo, returnPageData.totalPageCount);

  // preset 기간 버튼 클릭 시 해당 기간으로 재조회합니다.
  const handleSelectPresetRange = (range: Exclude<ShopMypageReturnRange, "custom">): void => {
    const monthMap: Record<Exclude<ShopMypageReturnRange, "custom">, 1 | 3 | 6> = {
      "1m": 1,
      "3m": 3,
      "6m": 6,
    };
    ...
    router.push(buildMypageReturnHref(range, presetDateRange.startDate, presetDateRange.endDate, 1));
  };
  ...
  const handleSearch = (): void => {
    ...
    router.push(buildMypageReturnHref(selectedRange, startDateInputValue, endDateInputValue, 1));
  };
  ...
  return (
    <section className={styles.cancelSection}>
      <header className={styles.cancelHeader}>
        <div>
          <h1 className={styles.cancelTitle}>반품내역</h1>
          <p className={styles.cancelDescription}>
            {returnPageData.startDate} ~ {returnPageData.endDate} 반품건수 {formatShopMypageOrderCount(returnPageData.returnCount)}
            건
          </p>
        </div>
      </header>
      ...
      <ShopMypageReturnCardList returnList={returnPageData.returnList} />

      {paginationState.totalPageCount > 0 ? (
        <nav className={styles.paginationWrap} aria-label="반품내역 페이지 이동">
          {paginationState.hasPrevPage ? (
            <Link
              className={styles.paginationArrow}
              href={buildMypageReturnHref(selectedRange, returnPageData.startDate, returnPageData.endDate, paginationState.prevPageNo)}
            >
              {"<"}
            </Link>
          ) : (
          ...
          {paginationState.hasPrevBlock ? (
            <Link
              ...
              href={buildMypageReturnHref(selectedRange, returnPageData.startDate, returnPageData.endDate, paginationState.prevBlockPageNo)}
            >
              ...
            </Link>
          ) : null}
          ...
          {paginationState.pageNoList.map((targetPageNo) =>
            targetPageNo === paginationState.currentPageNo ? (
              ...
              <Link
                ...
                href={buildMypageReturnHref(selectedRange, returnPageData.startDate, returnPageData.endDate, targetPageNo)}
              >
                {targetPageNo}
              </Link>
            ),
          ...
          {paginationState.hasNextBlock ? (
            <Link
              className={styles.paginationEllipsis}
              href={buildMypageReturnHref(selectedRange, returnPageData.startDate, returnPageData.endDate, paginationState.nextBlockPageNo)}
            >
              ...
            </Link>
          ) : null}
          ...
          {paginationState.hasNextPage ? (
            <Link
              className={styles.paginationArrow}
              href={buildMypageReturnHref(selectedRange, returnPageData.startDate, returnPageData.endDate, paginationState.nextPageNo)}
            >
              {">"}
            </Link>
          ) : (
