import { fetchShopMypageOrderExchangePageServerData } from "@/domains/mypage/api/mypageServerApi";
import ShopMypageOrderExchangeSection from "@/domains/mypage/components/ShopMypageOrderExchangeSection";
...
interface ShopMypageOrderExchangePageProps {
  params?: Promise<{
    ordNo?: string;
  }>;
...
// 주문번호와 주문상세번호 기준 교환 신청 경로를 로그인 복귀용 상대 경로로 생성합니다.
function buildShopMypageOrderExchangePath(ordNo: string, ordDtlNo: number | undefined): string {
  const normalizedOrdNo = ordNo.trim();
  if (typeof ordDtlNo !== "number") {
    return `/mypage/order/${encodeURIComponent(normalizedOrdNo)}/exchange`;
  }

  const searchParams = new URLSearchParams();
  searchParams.set("ordDtlNo", String(ordDtlNo));
  return `/mypage/order/${encodeURIComponent(normalizedOrdNo)}/exchange?${searchParams.toString()}`;
}

// 쇼핑몰 마이페이지 교환 신청 화면을 렌더링합니다.
export default async function ShopMypageOrderExchangePage({
  params,
  searchParams,
}: ShopMypageOrderExchangePageProps) {
  ...
  // 로그인된 요청의 세션 쿠키를 사용해 교환 신청 데이터를 조회합니다.
  const requestContext = await requireAuthenticatedShopRequestContext(buildShopMypageOrderExchangePath(ordNo, ordDtlNo));
  const orderExchangePageData = await fetchShopMypageOrderExchangePageServerData(
    ordNo,
    ordDtlNo,
    requestContext.cookieHeader,
  );

  // 유효하지 않은 교환 화면 진입이면 주문상세 페이지로 이동시킵니다.
  if (!orderExchangePageData) {
    return (
      <ShopMypageOrderInvalidRedirect
        fallbackHref={buildShopMypageOrderDetailHref(ordNo)}
        alertMessage="교환 신청 정보를 찾을 수 없어 주문상세로 이동합니다."
      />
    );
  }

  return <ShopMypageOrderExchangeSection orderExchangePageData={orderExchangePageData} initialOrdDtlNo={ordDtlNo} />;
}
