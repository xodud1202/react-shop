...
interface ShopMypageOrderPageProps {
  searchParams?: Promise<Record<string, string | string[] | undefined>>;
}

interface ShopMypageOrderResolvedSearchState {
  range: ShopMypageOrderRange;
  pageNo: number;
  startDate: string;
  endDate: string;
}
...
function resolveOrderRange(rawRange: string | string[] | undefined): ShopMypageOrderRange {
...
// URL 검색 파라미터를 주문내역 SSR 조회용 상태로 정리합니다.
function resolveOrderSearchState(searchParamMap: Record<string, string | string[] | undefined>): ShopMypageOrderResolvedSearchState {
  ...
  const presetRangeMap: Record<Exclude<ShopMypageOrderRange, "custom">, { startDate: string; endDate: string }> = {
...
export default async function ShopMypageOrderPage({ searchParams }: ShopMypageOrderPageProps) {
  ...
  return (
    <ShopMypageOrderSection
      key={`${orderSearchState.range}-${orderSearchState.startDate}-${orderSearchState.endDate}-${orderSearchState.pageNo}`}
      orderPageData={orderPageData}
      initialRange={orderSearchState.range}
    />
  );
}
